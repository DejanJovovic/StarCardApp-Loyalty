            ...
            <View className="rounded-2xl shadow-md p-4" style={{ backgroundColor: bg }}>
                ...
                <View className="flex-row items-center justify-between">
                    ...
                    <View
                        ...
                        style={{ backgroundColor: card.isActive ? 'rgba(255,255,255,0.18)' : 'rgba(0,0,0,0.18)' }}
                    >
                ...
                {/* Balance */}
                <View className="mt-4">
                    <Text className="text-white/80 text-xs tracking-wider font-medium">LOYALTY BALANCE</Text>
                    <View className="flex-row items-baseline space-x-2 mt-1">
                        <Text className="text-white text-4xl font-extrabold">{card.stamps.total}</Text>
                ...
                {/* Reward progress */}
                <View>
                    <Text className="text-white font-medium">{card.info.cardDescription ?? 'Reward progress'}</Text>
                    <Text className="text-white/90 text-sm font-semibold tracking-wide mt-1">{active} stamps</Text>
                    <Text className="text-white/90 text-xs font-semibold tracking-wide">{missing} missing</Text>
                </View>
                    ...
                    {Array.from({ length: REWARD_THRESHOLD }).map((_, i) => {
                        ...
                        return (
                            <View
                                ...
                                style={{
                                    backgroundColor: filled ? stampActiveBg ?? stampBg : stampBg ?? 'transparent',
                                    borderColor: filled ? stampActiveBg : stampInactiveBg,
                                }}
                            >
                                <Ionicons name="cafe-outline" size={16} color={filled ? stampText : stampInactiveBg} />
                            </View>
                        );
                    })}
                </View>
                ...
                {/* CTA / info */}
                <Text className="text-white/95 font-medium">{card.info.rewardDetails ?? 'Check our new offers'}</Text>

                {/* QR image */}
                <View className="mt-3 rounded-xl bg-white/10 items-center p-3">
                    <Image source={images.qrCodeTest} resizeMode="contain" className="w-full h-24" />
                </View>
            </View>
        </View>
    );
